...
/**
 * A map containing keys of heterogeneous values. The keys in this map must implement `HMapKey[V]`, where `V` is the
 * type of its associated key.
...
class HMap[KeyType[_] <: HMapKey[_]](val entries: ListBuffer[(KeyType[V], V) forSome { type V }]) {
  ...
  /**
   * If given key is already in this map, returns associated value. Otherwise, computes value from given
  ...
  /**
   * Adds a new key/value pair to this map. If the map already contains a mapping for the key, it will be
  ...
  /**
   * Creates a new map containing the key/value mappings provided by the specified `HMap` and
   * all the key/value mappings of this map.
   * @param other the map to append
   * @return a new map containing mappings of this map and those provided by `other`.
   */
   ...
   * @return a new collection resulting from applying the given function `f` to each element of this map and
