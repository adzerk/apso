...
object Implicits {
  ...
  final implicit class ApsoJsonJsValue(val jsValue: JsValue) extends AnyVal {
    ...
    /**
     * Unwraps a JSON value. If the given value is a JSON string, number or boolean, a `String`, `BigDecimal` or
     * `Boolean` is returned respectively. If the given value is a JSON array or object, a `List[Any]` or
     * `Map[String, Any]` is returned respectively, where each of the values is recursively unwrapped. If the
     * given value is a JSON null, `null` is returned.
