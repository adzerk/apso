...
object Implicits {
  ...
  final implicit class PimpedSeq[T](val seq: Seq[T]) extends AnyVal {
  ...
  final implicit class PimpedMap[A, B](val map: Map[A, B]) extends AnyVal {
  ...
  final implicit class PimpedListMap[K, V](val list: Seq[Map[K, V]]) extends AnyVal {
  ...
  final implicit class PimpedCloseable[U <: AutoCloseable](val res: U) extends AnyVal {
    def use[T](f: U => T): T =
