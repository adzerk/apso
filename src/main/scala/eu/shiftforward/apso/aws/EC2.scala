...
/**
 * A representation of Amazon's EC2 service. This class wraps an [[com.amazonaws.services.ec2.AmazonEC2Client]] and
 * provides a higher level interface for querying the currently running instances.
...
class EC2(credentials: AWSCredentials = CredentialStore.getCredentials) extends Logging {
   ...
   * @return the information about the instance with the given id wrapped in a `Some` if such an instance exists, `None`
   *         otherwise.
   */
  def instance(id: String): Option[Instance] = client.describeInstances(new DescribeInstancesRequest()
    .withInstanceIds(id)).getReservations.headOption.flatMap(_.getInstances.headOption)
